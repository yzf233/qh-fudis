...
public class MapFile {
  ...
  public static final String INDEX_DATA_FILE_NAME = "index.data";
  ...
  protected Object get(byte[] key) throws Exception {
    return (Object) windexMap.getB(key);
  }

  public void rmKey(byte[] key) throws Exception {
    windexMap.remove(key);
  }

  protected void putValue(byte[] key, long docNum) throws Exception {
    windexMap.put(key, docNum);
  }

  protected VHashMap getMap() throws Exception {
    return windexMap;
  }
  protected void init()
  {
    try {
      File dataFile = new File(mapDir, MAP_INDEX_DATA_FILE_NAME);
      if (dataFile.exists()) {
  ...
  protected MapFile(NutchFileSystem nfs, String dirName) throws IOException {
    this.nfs = nfs ;
    mapDir = dirName ;
  }
  protected void writeMap() throws Exception {
    NutchFileSystem nfs = NutchFileSystem.get() ;
    File dataFile = new File(mapDir, MAP_INDEX_DATA_FILE_NAME);
    WebDbIndex webIndex = new WebDbIndex();
    org.apache.nutch.io.LongWritable key = new LongWritable(0) ;
    windexWriter = new SequenceFile.Writer(nfs,dataFile.getPath() , LongWritable.class ,WebDbIndex.class, true);
  ...
  public static class Writer {
    ...
    private NFSDataOutputStream index_data_writer; //文档索引信息 ， 记录文档数量
    private NFSDataInputStream index_data_reader;
    public static long docNum = 0;
    private int indexInterval = 128;

    private long size = MapFile.Writer.docNum;
    ...
    public Writer(NutchFileSystem nfs, String dirName, Class keyClass,
    ...
    public Writer(NutchFileSystem nfs, String dirName, WritableComparator
      ...
      File index_dataFile = new File(dir, INDEX_DATA_FILE_NAME);
      if (index_dataFile.exists() && index_dataFile.length() > 0) {
        this.index_data_reader = new NFSDataInputStream(nfs.open(index_dataFile),
            1024);
        if (index_data_reader != null) {
          size = index_data_reader.readLong();
        }
      }
      else {
        size = 0;
      }
      MapFile.Writer.docNum = size;
      ...
      this.index_data_writer = new NFSDataOutputStream(nfs.open(index_dataFile, false));
    }
    ...
    public synchronized void close() throws IOException {
      data.close();
      index.close();
      index_data_writer.writeLong(MapFile.Writer.docNum);
      index_data_writer.close();
    }

    /** Append a key/value pair to the map.  The key must be strictly greater
     * than the previous key added to the map. */
    public synchronized void append(WritableComparable key, Writable val) throws
        IOException {
        checkKey(key);
        ...
        if(val instanceof WebDB){
          ((WebDB)val).setKey(size);
          ((WebDB)val).setIndex(data.getLength());
        }
        data.append(key, val); // append key/value to data
        size++;
        MapFile.Writer.docNum++;
    }

    /** Append a key/value pair to the map.  The key must be strictly greater
     * than the previous key added to the map. */
    public synchronized void update(long key, Writable val) throws IOException {
       //System.out.println("******:"+val);
       data.update(key, new LongWritable(key), val);
    }
  ...
  public static class Reader {
    ...
    public Reader(NutchFileSystem nfs, String dirName,
                  WritableComparator comparator) throws IOException {
      File dir = new File(dirName);
      File dataFile = new File(dir, DATA_FILE_NAME);
      File indexFile = new File(dir, INDEX_FILE_NAME);
      ...
      try {
        ...
        while (true) {
          ...
          if (lastKey != null && comparator.compare(lastKey, k) >= 0)
            throw new IOException("key out of order: " + k + " after " +
                                  lastKey);
          lastKey = k;
     ...
     */
    public synchronized void finalKey(WritableComparable key) throws
        IOException {
    ...
    public synchronized Writable get(WritableComparable key, Writable val) throws
        IOException {
      if (seek(key)) {
  ...
  public static void rename(NutchFileSystem nfs, String oldName, String newName) throws
      IOException {
  ...
  public static void delete(NutchFileSystem nfs, String name) throws
      IOException {
  ...
  public static long fix(NutchFileSystem nfs, File dir,
                         Class keyClass, Class valueClass, boolean dryrun) throws
      Exception {
    ...
    Writable key = (Writable) keyClass.getConstructor(new Class[0]).newInstance(new
        Object[0]);
    Writable value = (Writable) valueClass.getConstructor(new Class[0]).
        newInstance(new Object[0]);
    SequenceFile.Writer indexWriter = null;
    if (!dryrun)
      indexWriter = new SequenceFile.Writer(nfs, index.toString(), keyClass,
                                            LongWritable.class);
    try {
      long pos = 0L;
      LongWritable position = new LongWritable();
