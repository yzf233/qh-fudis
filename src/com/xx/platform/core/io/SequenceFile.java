...
public class SequenceFile {
  ...
  public static class Writer {
    ...
    public Writer(NutchFileSystem nfs, String name,
      ...
      {
          init0(new NFSDataOutputStream(nfs.open(target,true)),keyClass, valClass);
      }
    ...
    public Writer(NutchFileSystem nfs, String name,
      ...
      this.nfs = nfs;
      this.target = new File(name);
      if (!nfs.exists(target) || overWrite) {
          init(new NFSDataOutputStream(nfs.open(target,false)),
           keyClass, valClass);
      }else
      {
          init0(new NFSDataOutputStream(nfs.open(target,true)),keyClass, valClass);
    ...
    private void init0(NFSDataOutputStream out,
                      Class keyClass, Class valClass) throws IOException {
      this.out = out;
      this.keyClass = keyClass;
      this.valClass = valClass;
      this.count = MapFile.Writer.docNum ;
      SequenceFile.Reader reader = new SequenceFile.Reader(nfs, target.getCanonicalPath());
      if(reader!=null)
      {
    ...
    /** Append a key/value pair. */
    public void update(long docNo, Writable key, Writable val) throws IOException {
      if (key.getClass() != keyClass)
        throw new IOException("wrong key class: "+key+" is not "+keyClass);
      if (val.getClass() != valClass)
        throw new IOException("wrong value class: "+val+" is not "+valClass);
      ...
      long pos = out.getPos() ;
      out.seek(((WebDB)val).getIndex()) ;
      update(buffer.getData(), 0, buffer.getLength(), keyLength,docNo);
      out.seek(pos) ;
    }
    /** Append a key/value pair. */
    public void update(byte[] data, int start, int length, int keyLength ,long docNum)
      throws IOException {
      if (keyLength == 0)
        throw new IOException("zero length keys not allowed");

      if ((++docNum % SYNC_INTERVAL) == 0) {       // time to emit sync
        out.writeInt(SYNC_ESCAPE);                // escape it
        out.write(sync);                          // write sync
      }
