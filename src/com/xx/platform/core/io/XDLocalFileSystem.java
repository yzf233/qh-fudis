 ...
 *****************************************************************/
public class XDLocalFileSystem extends NutchFileSystem {
    ...
    /**
     */
    public XDLocalFileSystem() throws IOException {
        super();
    ...
    public NFSOutputStream open(File f, boolean append) throws IOException {
        return new LocalNFSFileOutputStream(f,append);
    }
    ...
    class LocalNFSFileOutputStream extends NFSOutputStream {
      XDRandomAccessFile fos;
      long length =0 ;
      public LocalNFSFileOutputStream(File f) throws IOException {
        this.fos = new XDRandomAccessFile(f,"rws",BUFF_SIZE);
      }
      public LocalNFSFileOutputStream(File f,boolean rewrite) throws IOException {
        this.fos = new XDRandomAccessFile(f,"rws",BUFF_SIZE);
        if(rewrite)
        {
            fos.seek(fos.length());
      ...
      public long position(long position) throws IOException {
          this.fos.seek(position);
          return fos.getFilePointer();
      }
    }
