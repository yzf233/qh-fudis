...
public class XDRandomAccessFile extends RandomAccessFile {

//    static ResourceBundle res = ResourceBundle.getBundle("com.xx.platform.core.io.XDRandomAccessFile.Res");
    ...
    public XDRandomAccessFile(String name) throws  IOException {
        this(name, "r", DEFAULT_BUFFER_BIT_LEN);
    }

    public XDRandomAccessFile(File file) throws IOException, FileNotFoundException {
        this(file.getPath(), "r", DEFAULT_BUFFER_BIT_LEN);
    }

    public XDRandomAccessFile(String name, int bufbitlen) throws  IOException {
        this(name, "r", bufbitlen);
    }

    public XDRandomAccessFile(File file, int bufbitlen) throws IOException, FileNotFoundException {
        this(file.getPath(), "r", bufbitlen);
    }

    public XDRandomAccessFile(String name, String mode) throws IOException {
        this(name, mode, DEFAULT_BUFFER_BIT_LEN);
    }

    public XDRandomAccessFile(File file, String mode) throws IOException, FileNotFoundException {
        this(file.getPath(), mode, DEFAULT_BUFFER_BIT_LEN);
    }

    public XDRandomAccessFile(String name, String mode, int bufbitlen) throws IOException  {
        super(name, mode);
        this.init(name, mode, bufbitlen);
    }

    public XDRandomAccessFile(File file, String mode, int bufbitlen) throws IOException, FileNotFoundException {
    ...
    public static void main(String[] args) throws IOException {
        ...
        {
            long readfilelen = 0;
            XDRandomAccessFile brafReadFile, brafWriteFile;

            brafReadFile = new XDRandomAccessFile("D:\\XDIo\\0");
            readfilelen = brafReadFile.initfilelen;
            brafWriteFile = new XDRandomAccessFile("D:\\XDIo\\0_r", "rw", 1024);
...
//            }
            brafWriteFile.seek(brafWriteFile.length());
            brafWriteFile.write("²©¿ÍÐÅÏ¢".getBytes());
            brafWriteFile.close();
            brafReadFile.close();
