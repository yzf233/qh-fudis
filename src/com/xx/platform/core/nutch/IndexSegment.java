...
public class IndexSegment {
  ...
  File outputIndex, tmpOutputIndex ;
  /**
  ...
  public void init() throws IOException {
      outputIndex = new File(srcDir, "index");
      tmpOutputIndex = new File(localWorkingDir, "index");
      File localOutput = nfs.startLocalOutput(outputIndex, tmpOutputIndex);
      writer
          = new IndexWriter(localOutput,
                            new NutchDocumentAnalyzer(), false);
  ...
  public void indexPages() throws Exception {
      ...
      writer.setRAMBufferSizeMB(MIN_MERGE_DOCS);
      ...
      try {
          ...
          for (count = 0; count < maxDocs; count++) {
            if (!sr.next(fetcherOutput, null, parseText, parseData)) continue;
              ...
              doc = IndexingFilters.filter(doc, parse, fetcherOutput ,null);
  ...
  private Document makeDocument(String segmentName, long docNo,
    ...
    System.out.println("boost:"+fo.getFetchListEntry().getPage().getURL().toString()+":"+fo.getFetchListEntry().getPage().getScore());
