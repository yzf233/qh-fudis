    ...
    for(int i=0;i<count;i++) {
      ...
      if (field.isIndexed() && doInvert) {
        ...
        if (fieldState.length > 0)
          fieldState.position += docState.getAnalyzer(field.name()).getPositionIncrementGap(fieldInfo.name);
        ...
        if (!field.isTokenized()) {		  // un-tokenized field
          ...
          if (streamValue != null) 
            ...
            stream = docState.getAnalyzer(field.name()).reusableTokenStream(field.name(), reader);
          }
