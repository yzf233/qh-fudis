...
public class IndexSearcher extends Searcher {
  IndexReader reader;
  private static Map delMap;//´æ·ÅÉ¾³ýµÄË÷Òýid
  ...
  public void search(Weight weight, Filter filter,
    ...
    if (filter == null&&getDelMap().size()==0) {
...
//    boolean more = filterDocIdIterator.next() && scorer.skipTo(filterDocIdIterator.doc());
    while(scorer.next())
    {
        ...
        if (getDelMap().get(String.valueOf(scorerDocId))==null) {
  ...
  public Explanation explain(Weight weight, int doc) throws IOException {
	...
	public static Map getDelMap() {
		if(delMap==null)
			delMap=new java.util.HashMap();
		return delMap;
	}
