 ...
 *
 * @since   lucene 1.4
 * @version $Id: Sort.java 695514 2008-09-15 15:42:11Z otis $
 */
public class Sort
...
//        new SortField(field, SortField.AUTO, reverse), SortField.FIELD_DOC };
	    ...
	    SortField[] nfields = new SortField[str.length];
		  try
		  {
	        for(int i=0;i<str.length;i++)
	        {
	    	    ...
	    	    if(s[1].equalsIgnoreCase("asc"))
	    	    	is=false;
	    	    if(s[1].equalsIgnoreCase("score"))
	    	    {
	    	    nfields[i]=SortField.FIELD_SCORE;
	    	    }
	    	    else
	    	    nfields[i]=new SortField(s[0], SortField.AUTO,is);
	        }
		  }
		  catch(Exception e)
		  {
			  nfields[str.length]=SortField.FIELD_SCORE;
		  }
//	    nfields[str.length]=SortField.FIELD_DOC;
//	    nfields[str.length]=SortField.FIELD_SCORE;
    fields = nfields;
  }
