...
public class DistributedWebDBWriter implements IWebDBWriter {
    ...
    private abstract class CloseProcessor {
        ...
        long closeDown(File workingDir, File outputDir) throws IOException {
            ...
            if (numEdits != 0) {
                ...
                File srcSectionDir = new File(oldDbDir, "dbsection." + machineNum);
                File srcDbFile = new File(srcSectionDir, basename);
                nfs.rename(srcDbFile, newDbFile);
    ...
    private class PagesByURLProcessor extends CloseProcessor {
        ...
        PagesByURLProcessor(MapFile.Reader db, EditSectionGroupWriter editWriter, EditSectionGroupWriter futureEdits) {
            super(PAGES_BY_URL, db, editWriter, new SequenceFile.Sorter(nfs, new PageInstruction.UrlComparator(), NullWritable.class), new UTF8.Comparator(), null, Page.class, "PagesByURLPart");
    ...
    private class PagesByMD5Processor extends CloseProcessor {
        ...
        PagesByMD5Processor(MapFile.Reader db, EditSectionGroupWriter editWriter) {
            super(PAGES_BY_MD5, db, editWriter, new SequenceFile.Sorter(nfs, new PageInstruction.PageComparator(), NullWritable.class), null, Page.class, NullWritable.class, "PagesByMD5Part");
        }
    ...
    private class LinksByMD5Processor extends CloseProcessor {
        ...
        public LinksByMD5Processor(MapFile.Reader db, EditSectionGroupWriter editWriter, EditSectionGroupWriter futureEdits) {
            super(LINKS_BY_MD5, db, editWriter, new SequenceFile.Sorter(nfs, new LinkInstruction.MD5Comparator(), NullWritable.class), new Link.MD5Comparator(), Link.class, NullWritable.class, "LinksByMD5Part");
            this.futureEdits = futureEdits;
        }
    ...
    public static void createDB(NutchFileSystem nfs, File root, int totalMachines) throws IOException {
        ...
        if (nfs.exists(machineInfo)) {
            throw new IOException("Cannot create DistributedWebDB at " + nfs + ", as it already exists.");
        }
        ...
        for (int i = 0; i < totalMachines; i++) {
            File webdbDir = new File(stdDir, "webdb");
            File sectionDir = new File(webdbDir, "dbsection." + i);
            ...
            new MapFile.Writer(nfs, pagesByMD5File.getPath(), new Page.Comparator(), NullWritable.class).close();
            new MapFile.Writer(nfs, linksByURLFile.getPath(), new Link.UrlComparator(), NullWritable.class).close();
            new MapFile.Writer(nfs, linksByMD5File.getPath(), new Link.MD5Comparator(), NullWritable.class).close();
        }
        ...
        out = new DataOutputStream(nfs.create(readyToUse));
        try {
    ...
    NutchFileSystem nfs;
    File root, dbDir, oldDbDir, newDbDir, tmpDir;
    File localWriteLock, globalWriteLock, closeCounter, openCounter;
    ...
    public DistributedWebDBWriter(NutchFileSystem nfs, File root, int machineNum) throws IOException {
        ...
        File stdDir = new File(root, "standard");
        this.dbDir = new File(stdDir, "webdb");
        this.oldDbDir = new File(stdDir, "webdb.old");
        this.newDbDir = new File(stdDir, "webdb.new");
        this.tmpDir = new File(newDbDir, "tmp");

        //
        // Wait indefinitely for "ready-to-use-flag".
        //
        File readyToUse = new File(stdDir, "readyToUse");
        while (! nfs.exists(readyToUse)) {
            try {
                Thread.sleep(2000);
        ...
        this.closeCounter = new File(newDbDir, "closeCounter");
        ...
        ****/
        ...
        if (nfs.exists(openCounter)) {
            in = new DataInputStream(nfs.open(openCounter));
            try {
                in.read();  // version
                numOpens = in.readInt();
            } finally {
        ...
        if (numOpens == 0) {
            ...
            EditSectionGroupWriter.createEditGroup(nfs, dbDir, PAGES_BY_MD5, totalMachines, EditSectionGroupWriter.MD5_KEYSPACE);
            EditSectionGroupWriter.createEditGroup(nfs, dbDir, LINKS_BY_URL, totalMachines, EditSectionGroupWriter.URL_KEYSPACE);
            EditSectionGroupWriter.createEditGroup(nfs, dbDir, LINKS_BY_MD5, totalMachines, EditSectionGroupWriter.MD5_KEYSPACE);

            // Remove the flag that tells readers it's OK to proceed
            File dirIsComplete = new File(dbDir, "dbIsComplete");
            nfs.delete(dirIsComplete);
        }

        // These are the NutchFiles for this section of the read-only
        // db.
        File sectionDir = new File(dbDir, "dbsection." + machineNum);
        File pagesByURL = new File(sectionDir, PAGES_BY_URL);
        File pagesByMD5 = new File(sectionDir, PAGES_BY_MD5);
        File linksByURL = new File(sectionDir, LINKS_BY_URL);
        File linksByMD5 = new File(sectionDir, LINKS_BY_MD5);
        ...
        this.pagesByMD5 = new MapFile.Reader(nfs, pagesByMD5.getPath(), new Page.Comparator());
        this.linksByURL = new MapFile.Reader(nfs, linksByURL.getPath(), new Link.UrlComparator());
        this.linksByMD5 = new MapFile.Reader(nfs, linksByMD5.getPath(), new Link.MD5Comparator());
        ...
        this.pagesByMD5Writer = new EditSectionGroupWriter(nfs, machineNum, totalMachines, PAGES_BY_MD5, PageInstruction.class, NullWritable.class, new EditSectionGroupWriter.PageMD5Extractor());
        this.linksByURLWriter = new EditSectionGroupWriter(nfs, machineNum, totalMachines, LINKS_BY_URL, LinkInstruction.class, NullWritable.class, new EditSectionGroupWriter.LinkURLExtractor());
        this.linksByMD5Writer = new EditSectionGroupWriter(nfs, machineNum, totalMachines, LINKS_BY_MD5, LinkInstruction.class, NullWritable.class, new EditSectionGroupWriter.LinkMD5Extractor());
    }
    ...
    public static void main(String argv[]) throws FileNotFoundException, IOException {
        if (argv.length < 2) {
            System.out.println("Usage: java org.apache.nutch.db.DistributedWebDBWriter (-local | -ndfs <namenode:port>) <root> [-create <numProcessors>] | <machineInt> ([-addpage id url] | [-addpageifnp id url] | [-deletepage url] | [-addlink fromID url] | [-deletelink fromID])");
            return;
        }
        ...
        if ("-create".equals(argv[i])) {
            i++;
            DistributedWebDBWriter.createDB(nfs, root, Integer.parseInt(argv[i++]));
            System.out.println("Created webdb at " + nfs + ", " + root);
        } else {
            int machineNum = Integer.parseInt(argv[i++]);
            String cmd = argv[i++];
            ...
            if ("-addpage".equals(cmd)) {
                ...
                DistributedWebDBWriter writer = new DistributedWebDBWriter(nfs, root, machineNum);
    ...
    //qh 07/07/07
    public com.xx.platform.core.nutch.WebDB addWebDB(Page page) throws IOException {
        return null;
    }
