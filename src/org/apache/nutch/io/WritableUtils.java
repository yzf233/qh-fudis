...
public final class WritableUtils  {
   ...
   * @throws IOException
   */
  public static int  writeByteArray(DataOutput out, byte[] bytes) throws IOException {
	if (bytes != null) {
	  out.writeInt(bytes.length);
	  out.write(bytes,0,bytes.length);
	  return bytes.length ;//((bytes.length != 0) ? (100*buffer.length)/bytes.length : 0);
	} else {
	  out.writeInt(-1);
	  return -1;
	}
   ...
   * @throws IOException
   */
  public static int  writeCompressedBufferedByteArray(DataOutput out, byte[] bytes) throws IOException {
	    if (bytes != null) {
	      ByteArrayOutputStream bos =  new ByteArrayOutputStream();
	      GZIPOutputStream gzout = new GZIPOutputStream(bos);
	      gzout.write(bytes,0,bytes.length);
