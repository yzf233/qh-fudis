...
//          System.out.println("写一个ID：server>"+call.id);
...
//          System.out.println(System.currentTimeMillis()-start);
  ...
  /**
   * 该方法同call()方法，只是带超时时间限制
   * @param param
   * @param address
   * @return
   * @throws IOException
   */
  public Writable call(Writable param, InetSocketAddress address,int theTimeout)
  throws IOException {
  Connection connection = getConnection(address,theTimeout);
  Call call = new Call(param);
  synchronized (call) {
    connection.sendParam(call);                 // send the parameter
    try {
      call.wait(60000);                       // wait for the result
    } catch (InterruptedException e) {
    	e.printStackTrace();
    }
  ...
  public Writable[] call(Writable[] params, InetSocketAddress[] addresses)
    ...
    synchronized (results) {
      for (int i = 0; i < params.length; i++) {
        ...
        try {
//          System.out.println("向每个节点发送参数..."+call.param);
  ...
  /** Get a connection from the pool, or create a new one and add it to the
   * pool.  Connections to a given host/port are reused. */
  private Connection getConnection(InetSocketAddress address,int timeout)
    throws IOException {
