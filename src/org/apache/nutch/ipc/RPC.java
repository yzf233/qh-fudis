...
public class RPC {
  ...
  private static class Invoker implements InvocationHandler {
    ...
    private int theTimeOut=0;
    public Invoker(InetSocketAddress address) {
      this.address = address;
    }
    public Invoker(InetSocketAddress address,int theTimeOut) {
        this.address = address;
        if(theTimeOut<0){
        	theTimeOut=0;
        }
    ...
    public Object invoke(Object proxy, Method method, Object[] args)
      throws Throwable {
    	if(theTimeOut==0){
    		...
    		//设置超时时间
    		ObjectWritable value = (ObjectWritable)CLIENT.call(new Invocation(method, args), address,theTimeOut);
    		return value.get();
    	}
    }
  }
   ...
   * @param protocol
   * @param addr
   * @param isTiomeOut：true为设置超时，false为无限
   * @return
   */
  public static Object getProxy(Class protocol, InetSocketAddress addr,int theTiomeOut) {
	    return Proxy.newProxyInstance(protocol.getClassLoader(),
	                                  new Class[] { protocol },
	                                  new Invoker(addr,theTiomeOut));
	  }
   ...
   * port. */
  public static Server getServer(final ServiceInterface instance, final int port) {
  ...
  public static Server getServer(final ServiceInterface instance, final int port,
    ...
    return new Server(port, Invocation.class, numHandlers,instance.getName()) {
...
//            Object[] parms =call.getParameters();
//            for (int i = 0; i < parms.length; i++) {
//				System.out.println("参数："+parms.toString());
//			}
            Object value = method.invoke(instance, call.getParameters());
//            if(value instanceof Hits){
//            System.out.println("返回结果："+((Hits)value).getLength());
//            }else{
//            	System.out.println("返回结果类型："+value.getClass());
//            }
