...
public abstract class Server {
  ...
  private class Listener extends Thread {
    ...
    public void run() {
      LOG.info(serviceName+" 启动，初始化监听端口：" + port +" 初始化连接池："+handlerCount);
      while (running) {
        try {
...
//      LOG.info(getName() + ": exiting");
  ...
  private class Connection extends Thread {
    ...
    public void run() {
//      LOG.info(serviceName+" 连接端口: " + port + " 客户端地址： "
//                   + socket.getInetAddress().getHostAddress() + " 状态: starting");
      try {
        while (running) {
          int id;
          try {
...
//        LOG.info(serviceName+" 连接端口: " + port + " 客户端地址： "
//                   + socket.getInetAddress().getHostAddress() + " 状态:exiting");
      }
    }
...
//      LOG.info(getName() + ": exiting");
   ...
   */
  protected Server(int port, Class paramClass, int handlerCount,String serviceName) {
    this.port = port;
