...
public class JobTracker
    ...
    ServiceInterface {
  private static final String SERVICE_NAME = "JobTracker ";
        ...
        synchronized (taskTrackers) {
          synchronized (trackerExpiryQueue) {
            ...
            while ( (trackerExpiryQueue.size() > 0) &&
                   ( (leastRecent = (TaskTrackerStatus) trackerExpiryQueue.
                      first()) != null) &&
              ...
              TaskTrackerStatus newProfile = (TaskTrackerStatus) taskTrackers.
  ...
  public MapOutputLocation[] locateMapOutputs(String taskId,
    ...
    JobInProgress job = (JobInProgress) jobs.get( (String) taskToJobMap.get(
        taskId));
  ...
  public class JobInProgress {
    ...
    public JobInProgress(String jobFile) throws IOException {
      ...
      String url = "http://" + localMachine + ":" + TRACKERINFO_PORT +
    ...
    public synchronized void completedTask(String taskid) {
      ...
      if ( (t = (Task) incompleteMapTasks.get(taskid)) != null) {
        ...
        // as just finishing
        throw new IllegalArgumentException("Impossible state: task " + taskid +
            " reported as complete, but was not known as incomplete");
      }
    ...
    public MapOutputLocation[] locateTasks(String taskIds[]) {
      ...
      for (int i = 0; i < taskIds.length; i++) {
        if (completeMapTasks.get(taskIds[i]) != null) {
          ...
          TaskTrackerStatus tracker = (TaskTrackerStatus) taskTrackers.get(
              trackerId);
          v.add(new MapOutputLocation(taskIds[i], tracker.getHost(),
                                      tracker.getPort()));
        }
      }
    ...
    public void updateTaskStatus(String taskid, TaskStatus status) {
      ...
      if (incompleteMapTasks.get(taskid) != null ||
        ...
        if (numMapTasks == 0) {
          ...
          this.status.setMapProgress( (float) (totalReportedMapProgress /
                                               numMapTasks));
        }
      }
        ...
        if (numReduceTasks == 0) {
          ...
          this.status.setReduceProgress( (float) (totalReportedReduceProgress /
                                                  numReduceTasks));
        }
      }
      else {
    ...
    void executeTask(String taskid) {
      ...
      updateTaskStatus(taskid,
                       new TaskStatus(taskid, 0.0f, TaskStatus.UNASSIGNED));
  ...
  Task getTaskAssignment(String taskTracker) {
    if (unassignedTasks.size() > 0) {
      ...
      JobInProgress job = (JobInProgress) jobs.get( (String) taskToJobMap.get(
          taskid));
      job.updateTaskStatus(taskid,
                           new TaskStatus(taskid, 0.0f, TaskStatus.RUNNING));
