...
public final class ParseData
  ...
  private Properties returnData ; //索引以后保存从数据库获取到的数据，然后返回给 BDB
  private ParseStatus status;
  private Set<CrawlRule> dataSet;
  private CrawlRule fetchField;
  private int isParseOutLink = 1; //默认的 ， 解析OutLink ， 地址发生器发生的URL 不解析OutLink
  public ParseData() {}

  private Dbtable dbtable;
  public ParseData(ParseStatus status, String title, Outlink[] outlinks,
                   Properties metadata, Dbtable dbtable) {
    this.status = status;
    this.title = title;
  ...
  public CrawlRule getFetchField() {
    return fetchField;
  }
  ...
  public final void readFields(DataInput in) throws IOException {
    ...
    isParseOutLink = in.readInt(); // read isParseOutLink
...
//        int dataSetNum = in.readInt();
//        for (int i = 0; i < dataSetNum; i++) {
//            fetchField.setCode(UTF8.readString(in));
//            fetchField.setValue(UTF8.readString(in));
//        }
...
//        for (CrawlRule fieldData : dataSet) {
//            UTF8.writeString(out, fieldData.getCode());
//            UTF8.writeString(out, fieldData.getValue());
//        }
  }
  ...
  public static void main(String argv[]) throws Exception {
    String usage =
        "ParseData (-local | -ndfs <namenode:port>) recno segment";
  ...
  public void setIsParseOutLink(int isParseOutLink) {
    this.isParseOutLink = isParseOutLink;
  }
...
public Properties getReturnData() {
	return returnData;
}

public void setReturnData(Properties returnData) {
