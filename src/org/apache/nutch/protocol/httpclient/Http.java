...
public class Http implements org.apache.nutch.protocol.Protocol {
    ...
    static int MAX_THREADS_PER_HOST = NutchConf.get().getInt(
            "fetcher.threads.per.host", 1);

    static int MAX_THREADS_TOTAL = NutchConf.get().getInt(
            "fetcher.threads.fetch", 10);

    static String AGENT_STRING = getAgentString();
    static long SERVER_DELAY = (long) (NutchConf.get().getFloat(
    ...
    public ProtocolOutput getProtocolOutput(String urlString) {
        try {
            return getProtocolOutput(new FetchListEntry(true,
                    new Page(urlString, 1.0f, 0, 0), new String[0]));
        ...
        try {
            ...
            try {
                if (!RobotRulesParser.isAllowed(url))
                    return new ProtocolOutput(null,
                                              new ProtocolStatus(ProtocolStatus.
                            ROBOTS_DENIED, url));
            } catch (Throwable e) {
            ...
            if (code == 200) { // got a good response
                ...
                return new ProtocolOutput(c,
                                          new ProtocolStatus(ProtocolStatus.GONE,
                        "Http: " + code + " url=" + url));
                ...
                return new ProtocolOutput(c,
                                          new ProtocolStatus(protocolStatusCode,
                        url));
                ...
                return new ProtocolOutput(c,
                                          new ProtocolStatus(ProtocolStatus.EXCEPTION,
                        "Http code=" + code + ", url="
                        + url));
            }
        } catch (Throwable e) {
    ...
    private static void configureClient() {
        ...
        Protocol dummyhttps = new Protocol("https",
                                           new DummySSLProtocolSocketFactory(),
                                           443);
        ...
        if (NTLM_USERNAME.length() > 0) {
            Credentials ntCreds = new NTCredentials(NTLM_USERNAME,
                    NTLM_PASSWORD, NTLM_HOST, NTLM_DOMAIN);
