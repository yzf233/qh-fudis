...
public class DistributedSearch {
  ...
  public static class Server  {
    ...
    public static void main(String[] args) throws Exception {
      ...
      org.apache.nutch.ipc.Server server = RPC.getServer(bean, port, 100, true);
      server.start();
      server.join();
    }
  ...
  public static class Client
      extends Thread implements Searcher, HitDetailer, HitSummarizer,
      HitContent, Runnable ,com.xx.platform.core.nutch.IndexInterface{
    ...
    public Client(String[] servers) throws IOException {
      this(readConfig(servers));
    }

    private static InetSocketAddress[] readConfig(String[] config)
      throws IOException {
    ...
    public void updateSegments() throws IOException {
      ...
      defaultAddresses=SearchContext.testSearchConnection();
      // build segmentToAddress map
      Object[][] params = new Object[defaultAddresses.length][0];
//      getSegmentsNames method args is null ,
      String[][] results =
      ...
      for (int i = 0; i < results.length; i++) {  // process results of call
        ...
        List<Diserver> dislist=SearchContext.getDiserverList();
        for (Diserver server : dislist) {
            if (server.getIpaddress()!=null&&server.getIpaddress().equals(addr.getAddress())) {
                server.setServerstatus(true);
                SearchContext.getDao().updateIObject(server);
            }
          }
        if(liveAddresses.size()<SearchContext.CONTROL.getDisNumer())
      ...
      if(count%100==0){
    	  LOG.info("分布式状态: "+liveServers+" servers, "+liveSegments+" segments.");
      }
      count++;
      if(count==999999999l){
    	  count=0;
    ...
    public Hits search(final Query query, final int numHits,
      ...
      long totalHits = 0;
      liveAddresses=SearchContext.testSearchConnection();
//      System.out.println("分布式搜索开始查询...... 查询服务器数："+liveAddresses.length);
...
//      System.out.println("查询服务器返回查询结果："+results.length+" 查询结果数："+results[0].getTotal());
      ...
      int ls=results.length;
      ls=ls>SearchContext.CONTROL.getDisNumer()?SearchContext.CONTROL.getDisNumer():ls;
      for (int i = 0; i < ls; i++) {
        Hits hits = results[i];
        if (hits == null) continue;
        totalHits += hits.getTotal();
        int theLength=hits.getLength();
        for (int j = 0; j < theLength; j++) {
          Hit h = hits.getHit(j);
          boolean isk=false;
          if(maxValue!=null){
        	  isk=((reverse || sortField == null) ? h.getSortValue().compareTo(maxValue) >= 0 : h.getSortValue().compareTo(maxValue) <= 0);
          }
          if (maxValue == null || isk) {
        	  if(maxValue == null){
        		  queue.add(new Hit(i, h.getIndexDocNo(), h.getSortValue(), h.getDedupValue(),h.getScore(),false));
        	  }else{
        		  queue.add(new Hit(i, h.getIndexDocNo(), h.getSortValue(), h.getDedupValue(),h.getScore()));
        	  }
...
//          LOG.info("Querying segments from search servers...");
    ...
    public boolean deleteDocuments(int ino, int docno) throws IOException {
    	...
    	return ((IndexInterface)
                RPC.getProxy(IndexInterface.class, liveAddresses[ino])).deleteDocuments(ino,docno);
    }

    public int deleteDocuments(String field,String value) throws IOException {
    	liveAddresses=SearchContext.testSearchConnection();
    	int total =0;
        for(InetSocketAddress address :liveAddresses){
            try{
                total += ((IndexInterface)
                          RPC.getProxy(IndexInterface.class, address)).
                        deleteDocuments(field, value);
            }catch(Exception e){}
        }
        //SearchContext.addNewIndex(diserver);
        return total;
    }
