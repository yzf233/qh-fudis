...
public class UpdateDatabaseTool {
     ...
     */
    public UpdateDatabaseTool(IWebDBWriter webdb, boolean additionsAllowed,
                              int maxCount) {
        this.webdb = webdb;
     ...
     */
    public void updateForSegment(NutchFileSystem nfs, String directory) throws
            IOException {
        ...
        try {
            ...
            fetch.seek(Fetcher.paseDataNum);
            parse.seek(Fetcher.paseDataNum);
            FetcherOutput fo = new FetcherOutput();
            ParseData pd = new ParseData();
            while (fetch.next(fo) != null) {
                parse.next(pd);
                RuntimeDataCollect.setProcess_num(count);
                if ((count % 5000) == 0) {
                    LOG.info("´¦ÀíÎÄµµ " + count);
                }
                ...
                } else if (fo.getProtocolStatus().getCode() ==
     ...
     */
    private void pageContentsUnchanged(FetcherOutput fetcherOutput) throws
            IOException {
    ...
    private void pageContentsChanged(FetcherOutput fetcherOutput,
        ...
        for (int i = 0; i < end; i++) {
            ...
            if (additionsAllowed) {
                ...
                boolean internal = destHost == null ||
                                   destHost.equals(sourceHost);
                ...
                try {
                    ...
                    //
                    Link newLink = new Link(newPage.getMD5(), sourceDomainID,
                                            url, link.getAnchor());
                    ...
                    Page linkedPage = new Page(url, newScore, newNextScore,
                                               nextFetch);
                    if (parseData.getIsParseOutLink() == 1) {
...
//        webdb.addPage(newPage); // update record in db
     ...
     */
    private void pageGone(FetcherOutput fetcherOutput) throws IOException {
//        Page oldPage = fetcherOutput.getFetchListEntry().getPage();
//        Page newPage = (Page) oldPage.clone();
//
...
//        webdb.addPage(newPage); // update record in db
     ...
     */
    private void pageRetry(FetcherOutput fetcherOutput) throws IOException {
//        Page oldPage = fetcherOutput.getFetchListEntry().getPage();
//        Page newPage = (Page) oldPage.clone();
//
