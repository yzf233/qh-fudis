...
public class LogFormatter extends Formatter {
  ...
  static {
    ...
    if(LogFormatter.getLogger(LogFormatter.class.toString()).getParent()!=null)
    {
      handlers = LogFormatter.getLogger(LogFormatter.class.toString()).
          getParent().
          getHandlers();
      for (int i = 0; i < handlers.length; i++) {
        handlers[i].setFormatter(new LogFormatter());
  ...
  public static Logger getLogger(String name) {
    ...
    try {
        if(logger==null)
        {
            FileHandler fileHandler = new FileHandler(".."+File.separator+"logs"+File.separator+"searchserver.log",10240,1,true);
